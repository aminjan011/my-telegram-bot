...
# ==================== НАСТРОЙКИ ====================
BOT_TOKEN = "8646029465:AAHDm5rvMgp53-Hnut5DTgy5fgFsTlyga5U"      # Tokeningiz
...
ADMIN_USERNAME = "softic00"                    # Admin username (sans @)
ADMIN_ID = 1112793157                                  # Admin Telegram ID raqami
# ===================================================
...
def get_user(user_id: int):
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, referrer_id, points, has_access FROM users WHERE user_id = ?", (user_id,))
    row = cursor.fetchone()
    conn.close()
    return row
...
async def free_channel_handler(callback: CallbackQuery):
    ...
    if points >= REQUIRED_REFERRALS:
        try:
            ...
            text += "🎉 <b>Вы успешно собрали 10 баллов!</b>\n\n⚠️ <i>Внимание: Ссылка ниже одноразовая и действительна в течение 10 минут только для 1 человека! Не пересылайте её никому.</i>"
        except Exception as e:
